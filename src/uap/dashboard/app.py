...
Moved into uap package — all imports are now from uap.*
"""
...
from uap.protocol import StateManager, ACT, get_uap_home
from uap.dispatcher import Dispatcher, AgentConfig

# Paths — use UAP home for storage, package dir for local assets
DASHBOARD_DIR = Path(__file__).parent.resolve()
UAP_HOME = get_uap_home()
USER_DATA_FILE = UAP_HOME / "user_data.json"
...
def render_stats_page():
    ...
    # Load all ACT session files from UAP home
    act_dir = UAP_HOME / "sessions"
    ...
    for s in sessions:
        for log_entry in s.get("handshake_log", []):
...
def init_state():
    ...
    for k, v in defaults.items():
        if k not in st.session_state:
            if k == "state_mgr":
                st.session_state[k] = StateManager()
            elif k == "dispatcher":
