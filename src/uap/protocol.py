...
def get_sessions_dir() -> Path:
    """Get sessions storage directory (~/.uap/sessions)"""
    sessions_dir = get_uap_home() / "sessions"
...
class StateManager:
    ...
    def __init__(self, storage_dir: Optional[Path] = None):
        self.storage_dir = storage_dir or get_sessions_dir()
            ...
            try:
                with open(filepath, "r") as f:
                    ...
                    sessions.append({
                        ...
                        "agents": len(set(log.get("agent") for log in data.get("handshake_log", []))),
                        "tasks": len(data.get("task_chain", []))
                    })
